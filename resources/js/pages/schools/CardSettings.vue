...
import { Ear, Flame, LoaderCircle, Palette, Plus, Trash2, TriangleAlert, UserCheck } from 'lucide-vue-next';
import { computed, ref } from 'vue';

/**
 * Ontwikkeling → Spelerskaart.
 *
...
// Staat in het menu onder Ontwikkeling. Die groep heeft geen eigen pagina,
// dus de kruimel ervoor gaat naar het dashboard.
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Ontwikkeling', href: '/dashboard' },
    { title: 'Spelerskaart', href: '/instellingen/spelerskaart' },
];
...
const bevestig = () => {
    bezig.value = true;
    router.post('/instellingen/spelerskaart/variant', { mode: gekozen.value }, { preserveScroll: true, onFinish: () => (bezig.value = false) });
};
                ...
                <div v-if="wisselt" class="mt-4 rounded-xl border border-warning/40 bg-warning/5 p-4" role="alert">
                    ...
                    <ul v-if="gekozen === 'inzet'" class="mt-2 list-disc space-y-1 pl-5 text-sm text-muted-foreground">
                        ...
                        <li>
                            ...
                            <template v-if="counts.reports"
                                >Punten uit {{ counts.reports }} rapporten tellen niet meer mee, dus sommige kaarten zakken een level.</template
                            >
                        </li>
                        <li>Rapporten worden niet verwijderd. Stap je later terug, dan staan ze er weer.</li>
                        <li>
                            Trainers geven na de training inzetpunten in plaats van rapporten, en leggen per cursus een begin- en eindniveau vast.
                        </li>
                    </ul>
                    <ul v-else class="mt-2 list-disc space-y-1 pl-5 text-sm text-muted-foreground">
                        ...
                        <li>
                            Inzetpunten<template v-if="counts.courses"> en {{ counts.courses }} cursusniveaus</template> worden niet verwijderd, maar
                            ouders zien ze niet meer. Stap je later terug, dan staan ze er weer.
                        </li>
            ...
            <form class="mt-8 space-y-5" @submit.prevent="opslaan">
                ...
                <template v-else>
                    ...
                    >
                        ...
                        <p class="mt-1 text-sm text-muted-foreground">
                            {{ soort.uitleg }} Drie of vier niveaus, elk met punten. Er is geen negatieve keuze.
                        </p>

                        <div class="mt-3 space-y-2">
                            <div v-for="(trede, i) in form[soort.veld]" :key="i">
                                ...
                                <InputError class="mt-1" :message="fout(`${soort.veld}.${i}.label`) ?? fout(`${soort.veld}.${i}.points`)" />
                            </div>
                        </div>
                        <button
                        ...
                        <InputError class="mt-2" :message="fout(soort.veld)" />
                    </section>
                    ...
                    <section class="rounded-xl border border-border bg-card p-5 shadow-sm">
                        ...
                        <div class="mt-3 space-y-2">
                            <div v-for="(niveau, i) in form.progress_levels" :key="i">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="inline-block size-4 shrink-0 rounded-full"
                                        :style="{ backgroundColor: kleurVan(niveau.color) }"
                                    ></span>
                                    <input
                                        v-model="niveau.label"
                                        type="text"
                                        maxlength="20"
                                        :class="invoer"
                                        class="min-w-0 flex-1"
                                        :aria-label="'Naam kleur ' + (i + 1)"
                                    />
                                ...
                                <InputError class="mt-1" :message="fout(`progress_levels.${i}.label`) ?? fout(`progress_levels.${i}.color`)" />
                            </div>
                        </div>
                        <button
                        ...
                        <InputError class="mt-2" :message="fout('progress_levels')" />
